...
export function renderNavbar(activePage: 'home' | 'eventos' | 'inscripcion' | 'reglamento' = 'home'): string {
    ...
    .map(
      ...
      <a href="${link.href}"
         class="px-4 py-2 rounded-lg font-semibold transition-all duration-300 ${
           activePage === link.key
             ? 'bg-accent text-primary shadow-glow-yellow'
             : 'text-primary hover:bg-surface-warm hover:text-red'
         }">
        ${link.label}
      </a>`
    )
  ...
  return `
    <header>
      ...
      <nav class="sticky top-0 z-50 border-b border-primary/10 bg-white/95 backdrop-blur-md shadow-sm">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6">
          ...
          <button id="mobile-menu-btn" class="md:hidden p-2 text-primary" aria-label="Menú">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
            </svg>
          </button>
        </div>
        <div id="mobile-menu" class="hidden md:hidden border-t border-primary/10 px-4 py-3 space-y-1 bg-white">
          ${navLinks}
