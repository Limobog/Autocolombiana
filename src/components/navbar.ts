...
export function renderNavbar(activePage: 'home' | 'eventos' | 'inscripcion' | 'reglamento' = 'home'): string {
    ...
    .map(
      ...
      <a href="${link.href}"
         class="px-4 py-2 rounded-lg font-medium transition-all duration-300 ${
           activePage === link.key
             ? 'bg-secondary/20 text-accent shadow-glow-yellow'
             : 'text-gray-light hover:text-accent hover:bg-secondary/10'
         }">
        ${link.label}
      </a>`
    )
  ...
  return `
    <nav class="sticky top-0 z-50 border-b border-gray-metal/30 bg-primary/95 backdrop-blur-md">
      <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6">
        <a href="./index.html" class="flex items-center gap-3">
          ...
          <div class="hidden sm:block">
            <p class="font-title text-xl tracking-wider text-accent leading-none">LIMObog</p>
            <p class="text-xs text-gray-metal font-medium">Liga de Motociclismo de Bogotá</p>
          </div>
        </a>
        <div class="hidden md:flex items-center gap-1">${navLinks}</div>
        <button id="mobile-menu-btn" class="md:hidden p-2 text-accent" aria-label="Menú">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
          </svg>
        </button>
      </div>
      <div id="mobile-menu" class="hidden md:hidden border-t border-gray-metal/30 px-4 py-3 space-y-1">
        ${navLinks}
      </div>
    </nav>`;
}
